...
const stats = [
  { icon: Users, value: '300+', label: 'Unternehmen begleitet' },
  { icon: Bot, value: 'KI-Experte', label: 'Automatisierung & Strategie' },
  { icon: Handshake, value: 'Persönlich', label: 'bis zum konkreten Ergebnis' },
]
        ...
        <div className="grid md:grid-cols-[280px_1fr] gap-10 items-start mt-8">
          ...
          <div>
            ...
            <p className="font-mono text-[11px] font-medium tracking-wider uppercase text-[#2563EB] mb-5">
              KI-Berater & Automatisierungsexperte
            </p>

            <p className="text-[15px] text-[#555] dark:text-[#AAA] leading-relaxed mb-8">
              Michael begleitet KMUs dabei, operative Engpässe mit KI und Automatisierung zu lösen — kein Tool-Hype, sondern messbare Verbesserungen im Tagesgeschäft. Von effizienteren Abläufen bis zu skalierbaren Systemen, die Unternehmen spürbar entlasten.
            </p>

            <div className="grid grid-cols-3 gap-4">
              {stats.map((s) => (
                <div key={s.label} className="bg-[#F7F7F5] dark:bg-white/5 rounded-xl p-4 border border-[#EDEDEA] dark:border-white/10">
                  <s.icon size={18} className="text-[#2563EB] mb-2" />
                  <div className="font-heading text-base font-bold text-[#1A1A1A] dark:text-white mb-1">{s.value}</div>
                  <div className="text-[11px] text-[#999] dark:text-[#666] leading-snug">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
